...
#[cfg(feature = "ssr")]
use crate::models::{GlobalMetrics, HealthStatus, NodeMetrics, RankMetrics};
#[cfg(feature = "ssr")]
        ...
        match result {
            Ok((ranks, nodes)) => {
                ...
                for rank in &ranks {
                    ...
                    println!(
                        "Rank {}: IP={}, Status={:?}",
                        rank.rank_id, rank.node_ip, rank.status
                    );
                }
                ...
                for node in &nodes {
                    ...
                    println!(
                        "Node {}: {} ranks, status={:?}",
                        node.node_ip, node.rank_count, node.status
                    );
                }
            }
            Err(e) => {
...
pub fn aggregate_ranks_to_node_metrics(
    node_ip: &str,
    ranks: &[RankMetrics],
) -> Option<NodeMetrics> {
    if ranks.is_empty() {
    ...
    let healthy_count = ranks
        .iter()
        .filter(|r| r.status == HealthStatus::Healthy)
        .count() as u8;
    let warning_count = ranks
        .iter()
        .filter(|r| r.status == HealthStatus::Warning)
        .count() as u8;
    let critical_count = ranks
        .iter()
...
#[cfg(feature = "ssr")]
/// 生成全局聚合指标
pub fn generate_global_metrics_from_real_data(
    nodes: &[NodeMetrics],
    ranks: &[RankMetrics],
) -> GlobalMetrics {
...
#[cfg(feature = "ssr")]
use crate::models::{
    GlobalStepMetrics, RankStepMetrics, StepQueryRawResponse, StepQueryRequest, StepQueryResponse,
};
