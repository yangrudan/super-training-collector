...
Super Training Collector 是面向大规模分布式训练场景的可视化监控与诊断平台，适合在项目汇报、方案演示、运维看板和性能优化复盘中展示训练系统运行状态。

### 宣传亮点

- **一屏总览训练态势**：聚合健康分布、全局 KPI、训练进度和 HANG 状态，快速呈现任务整体运行情况。
- **千卡规模快速下钻**：支持从全局到节点、Rank 的三级视图，帮助观众理解异常定位路径。
- **性能瓶颈可视化**：结合 Step Time、GPU 利用率、NCCL 延迟和慢节点占比，直观展示性能治理价值。
- **火焰图辅助诊断**：提供调用栈采集与火焰图分析能力，便于展示从监控到根因分析的闭环。
- **演示友好**：内置模拟数据模式，可在无真实训练任务时用于产品介绍和功能演示。
...
| 性能优化 | 对比关键指标并定位慢节点、慢 Rank 和通信瓶颈 |
| 问题复盘 | 结合火焰图与 Step 记录说明问题定位过程 |
